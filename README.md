...
3. CLI для выгрузки в файл:
   ...
   ```
   Пример выгрузки всех объявлений по аренде апартаментов в Дана́нге через keyword-фильтр:
   ```bash
   python -m chotot.cli --region 32 --cg 1000 --cgr 1002 --keyword "căn hộ" --all-pages --output danang_apartments.json
   ```
